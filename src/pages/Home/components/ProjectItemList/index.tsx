 ...
 */
export const ProjectItemList = forwardRef<ProjectItemListRef>(function ProjectItemList(_props, ref): JSX.Element {
  // 项目数据
  const data: ProjectItemProps[] = [
    ...
    },
    {
      ...
      details: {
        name: 'COA Generator',
        description: 'Desktop client for generating Certificate of Analysis in an analytical testing laboratory.',
        techStack: ['Flutter'],
      },
    },
    {
      ...
      details: {
        name: 'Digifly',
        description: 'Data visualization web app with 7 pages presented on 9 large screens in a bank lobby.',
  ...
  // 项目组件的引用
  const projectItemRefs = data.map<NamedProjectItemRef>((it: ProjectItemProps) => {
    return {
    ...
    () => {
      return {
        ...
        scrollToProjectItem(projectName: string, arg?: boolean | ScrollIntoViewOptions | undefined): void {
          const namedRef = projectItemRefs.find(({ name }) => name === projectName)
